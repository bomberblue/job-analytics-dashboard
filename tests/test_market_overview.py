...
class TestFetchHeadlineMetrics(unittest.TestCase):
    ...
    def test_unfiltered_shape(self):
        ...
        # Generous ceiling, not a precise pin: the underlying dataset grows as the
        # pipeline is re-run, but the source has ~1.05M raw rows, so a count near
        # 2M would mean the load pipeline duplicated the table again (a bug this
        # project has hit before), not legitimate growth.
        self.assertLess(df.iloc[0]['total_postings'], 2_000_000)
